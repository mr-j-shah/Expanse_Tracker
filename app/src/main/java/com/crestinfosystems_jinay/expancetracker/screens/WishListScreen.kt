...
@Composable
fun WishListScreen(navController: NavController, viewModel: MainScreenViewModel) {
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(
                ...
                onClick = {
                    navController.navigate(ScreenRoute.AddWishScreen.route + "/0")
                },
        ...
        Column(modifier = Modifier.padding(it)) {
            ...
            val wishListData = viewModel.getAllWishes.combine(flowOf(searchText)) { list, query ->
                list.filter {
                    it.title.contains(query, ignoreCase = true) || it.desc.contains(
                        query,
                        ignoreCase = true
                    )
                }
            ...
            LazyColumn() {
                ...
                items(wishListData.value, key = { wish -> wish.id }) { wish ->
                    ...
                    SwipeToDismiss(
                        ...
                        wishItem(wish = wish) {
                            navController.navigate(ScreenRoute.AddWishScreen.route + "/${wish.id}")
                        }
                    }
